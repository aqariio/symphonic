...
public class CopperHornItem extends Item {
    ...
    public static ItemStack getStackForInstrument(Item item, RegistryEntry<SymphonicInstrument> instrument) {
        ItemStack itemStack = new ItemStack(item);
        CopperHornItem.setInstrument(itemStack, instrument);
        return itemStack;
    }

    public static void setRandomInstrumentFromTag(ItemStack stack, TagKey<SymphonicInstrument> instrumentTag, Random random) {
        Optional<RegistryEntry<SymphonicInstrument>> optional = SymphonicRegistries.INSTRUMENT.getEntryList(instrumentTag).flatMap(entryList -> entryList.getRandom(random));
    ...
    private static void setInstrument(ItemStack stack, RegistryEntry<SymphonicInstrument> instrument) {
    ...
    public void appendStacks(ItemGroup group, DefaultedList<ItemStack> stacks) {
        if (this.isIn(group)) {
            for (RegistryEntry<SymphonicInstrument> registryEntry : SymphonicRegistries.INSTRUMENT.iterateEntries(this.instrumentTag)) {
                stacks.add(CopperHornItem.getStackForInstrument(Symphonic.COPPER_HORN, registryEntry));
            }
        }
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        Optional<RegistryEntry<SymphonicInstrument>> optional = this.getInstrument(itemStack);
        if (optional.isPresent()) {
    ...
    public int getMaxUseTime(ItemStack stack) {
        Optional<RegistryEntry<SymphonicInstrument>> optional = this.getInstrument(stack);
        return optional.map(copperHornInstrumentRegistryEntry -> copperHornInstrumentRegistryEntry.value().useDuration()).orElse(0);
    }

    private Optional<RegistryEntry<SymphonicInstrument>> getInstrument(ItemStack stack) {
        ...
        Iterator<RegistryEntry<SymphonicInstrument>> iterator = SymphonicRegistries.INSTRUMENT.iterateEntries(this.instrumentTag).iterator();
        if (iterator.hasNext()) {
